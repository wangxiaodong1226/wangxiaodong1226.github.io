...
3.**使用window.name来进行跨域**；window对象有个name属性，该属性有个特征：即在一个窗口(window)的生命周期内，窗口载入的所有页面都是共享一个window.name的，每个页面对window.name都有读写的权限，window.name是持久
存在一个窗口载入过的所有页面中的，并不会因新页面的载入而进行重置。
4.**使用HTML5中新引入的window.postMessage方法来跨域传送数据**，使用它来自其他的window对象发送消息，无论这个window对象是属于同源或不同源，目前IE8+、FireFox、Chrome、Opera等浏览器都已经支持window.postMessage方法
5.**CORS** 
...
2.同一台服务器，但是你在https://github.com请求的是https://github2.com上的资源
---
...
`利用script标签没有浏览器跨域限制的漏洞，网页可以得到从其他来源动态产生的JSON数据。JSONP请求一定需要对方的服务器做支持才可以。`

优缺点:
1.【优点】Ajax和JSONP都是客户端向服务端发送请求，从而获取数据的方式。但是Ajax属于同源策略，而JSONP属于非同源策略。
...
该属性`表示哪些域名可以访问资源，如果设置通配符则表示所有网站都可以访问资源`。
...
Websocket是一种双向通信协议，在建立连接之后，Websocket的server与client都能主动向双方发送或接受数据。
![java-javascript](/img/网络/16.png)
